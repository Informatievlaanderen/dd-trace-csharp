namespace Be.Vlaanderen.Basisregisters.DataDog.Tracing.Tests
{
    using System;
    ...
    public class SpanTests
    {
        ...
        public void Span_end_should_seal_span()
        {
            ...
            {
                span.Invoking(t => t.SetMeta("test", "test")).Should().NotThrow();
                span.Invoking(t => t.SetError(new Exception())).Should().NotThrow();
                span.Invoking(t => t.Begin("foo", "bar", "foo", "bar")).Should().NotThrow();

                span.Dispose();

                span.Invoking(t => t.SetMeta("test", "test")).Should().Throw<InvalidOperationException>();
                span.Invoking(t => t.SetError(new Exception())).Should().Throw<InvalidOperationException>();
                span.Invoking(t => t.Begin("foo", "bar", "foo", "bar")).Should().Throw<InvalidOperationException>();
                span.Invoking(t => t.Dispose()).Should().Throw<InvalidOperationException>();
            }
